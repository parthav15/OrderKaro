...
async function resolveOption(restaurantId: string, itemId: string, custId: string, optId: string) {
...
export async function PUT(
  ...
  try {
    const { id: restaurantId, itemId, custId, optId } = await params
    requireRole(request, "OWNER", "MANAGER")
    await resolveOption(restaurantId, itemId, custId, optId)
