...
- **Fees = the platform margin.** `PlatformConfig` singleton (super-admin at `/admin/super/fees`, API `api/v1/admin/platform-config`): `deliveryFee` + `convenienceFee`, each `FLAT`|`PERCENT`, with `minFee`/`maxFee`. **Delivery-only** — `computePlatformFees(subtotal, orderType)` in `lib/platform-fees.ts`. **Not itemized to the diner** — bundled into a single "Delivery & handling" line. This total becomes the Stripe `application_fee_amount` / the Cashfree platform residual, carried through the gateway as `CheckoutRequest.platformFee` (previously always 0; `supportsPlatformFee` was false on every adapter).
